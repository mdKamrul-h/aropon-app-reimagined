...
import type { Installment, Loan } from '@/types/schema';
...
export default function LoanDetailScreen() {
  ...
  const load = async () => {
    ...
    const found = loans.find((l) => l.id === String(id)) ?? null;
    setLoan(found);
    if (found) setInstallments(await repo.getInstallments(found.id));
    setLoading(false);
  };
  ...
  const schedule = loan ? toComputedSchedule(loan, installments) : [];
  const next = loan ? nextInstallment(loan, installments) : null;
  const paid = loan ? loanPaidAmount(loan, installments) : 0;
          ...
          <>
            <SurfaceCard style={styles.headerCard}>
              ...
              <Text style={[styles.meta, { color: t.muted }]}>
                ...
                {loan.interest_rate ? ` · ${toBnDigits(loan.interest_rate)}% সুদ` : ''}
              </Text>
              <Text style={[styles.meta, { color: t.muted }]}>
                মোট পরিশোধযোগ্য ৳{toBnDigits(Math.round(total))}
              </Text>
              ...
              <Text style={[styles.progressText, { color: t.muted }]}>
                ৳{toBnDigits(Math.round(paid))} / ৳{toBnDigits(Math.round(total))} · {toBnDigits(pct)}%
              </Text>
            </SurfaceCard>
                  ...
                  <SurfaceCard key={inst.index} style={styles.instRow}>
                    ...
                    <View style={styles.instBody}>
                      <Text style={[styles.instTitle, { color: t.ink }]}>
                        কিস্তি #{toBnDigits(inst.index)} · ৳{toBnDigits(Math.round(inst.amount))}
                      </Text>
